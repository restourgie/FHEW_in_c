...
void FFTforward(double complex res[],double complex res2[], int val[]) {
  double complex data[DOUBLE_N];
  //fill array with two ffts of length 2048 
  //first one is -1,1,1,1,...,1,0,0,...,0
  //second one is -1,1,...,2,1,0,...,0
  for(int i=0;i<N;++i){
    data[i] = 1 + I*1;
    data[i+N] = 0;
  }
  ...
  // }
  //untangle them.. IT WORKS!! ONLY NOT YET WORKING FOR res[0] and res2[0]
  for(int i=0;i<DOUBLE_N;++i){
    res[i] = (data[i] + conj(data[DOUBLE_N-i]))/2;
    res2[i] = (data[i] - conj(data[DOUBLE_N-i]))/(2*I);
  }
...
int main()
{
  ...
  double complex data[DOUBLE_N];
  double complex data2[DOUBLE_N];
  int buf[1];

  FFTforward(data,data2,buf);
