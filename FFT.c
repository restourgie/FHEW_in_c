...
void print_complx(const cplx_ptr *x,int dim){
  for (int i = 0; i < dim; ++i)
...
void init_wortel(int num,int lo,int level,double complex root)
{ 
  if(num > 1){
    int m = num/2;
    wortel[0][level][lo/num] = creal(root);
    wortel[1][level][lo/num] = cimag(root);
    wortel[2][level][lo/num] = -cimag(root);
    --level;
...
void FFTforward(Ring_FFT res, Ring_ModQ val)
{
  ...
  for (int i = 0; i < CPLXDIM; ++i)
    res[i] = vec_x.real[i] + I*vec_x.imag[i];
}
...
void FFTbackward(Ring_ModQ res, Ring_FFT val)
{
  for (int i = 0; i < CPLXDIM; ++i)
  {
    vec_x.real[i] = creal(val[i])/CPLXDIM;
    vec_x.imag[i] = cimag(val[i])/CPLXDIM;
  }
  ...
  for (int i = 0; i < CPLXDIM; ++i,++j)
  {
    res[i] = (long int) round(vec_x.real[i]);
    res[j] = (long int) round(vec_x.imag[i]);
  }
}
